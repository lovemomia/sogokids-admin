...
public class SubjectSkuServiceImpl implements SubjectSkuService {
    ...
    public List<SubjectSku> getEntitys() {
        ...
        String sql = "select Id,SubjectId,`Desc`,Price,OriginalPrice,Adult,Child,CourseCount,Time,TimeUnit,Status,AddTime from SG_SubjectSku where Status > ? and courseId = ? ";
        Object [] params = new Object[]{Quantity.STATUS_ZERO, Quantity.STATUS_ZERO};
    ...
    public List<SubjectSku> getModelsBySub_Key(int sub_id) {
        List<SubjectSku> reData = new ArrayList<SubjectSku>();
        String sql = "select Id,SubjectId,`Desc`,Price,OriginalPrice,Adult,Child,CourseCount,Time,TimeUnit,Status,AddTime from SG_SubjectSku where SubjectId = ? and courseId = ? and Status > ? ";
        Object [] params = new Object[]{sub_id, Quantity.STATUS_ZERO, Quantity.STATUS_ZERO};
        List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, params);
        if(list.size() > 0){
            for (int i = 0; i < list.size(); i++) {
                SubjectSku entity = new SubjectSku();
    ...
    public SubjectSku formEntity(HttpServletRequest request, int sub_id, int id) {
        ...
        entity.setCourseCount(Integer.parseInt(request.getParameter("courseCount")));
    ...
    public String getCourseSkuHtml(int course_id){
        ...
        String p_start = "<p>";
        String p_end = "</p>";
        String sql = "select Id,SubjectId,`Desc`,Price,OriginalPrice,Adult,Child,CourseCount,Time,TimeUnit,Status,AddTime from SG_SubjectSku where courseId = ? and Status > ? ";
        ...
        if (list.size() > 0){
            ...
            for (int i = 0; i < list.size(); i++) {
                sb.append(p_start);
                sb.append("原价:" + list.get(i).get("OriginalPrice"));
                sb.append("&nbsp;&nbsp;");
                sb.append("&nbsp;&nbsp;");
...
//                sb.append("课程数:"+list.get(i).get("CourseCount"));
                sb.append("&nbsp;&nbsp;");
                sb.append("&nbsp;&nbsp;");
                int timeUnit = Integer.parseInt(list.get(i).get("TimeUnit").toString());
                String timeUnitName = "年";
                if (timeUnit == 1){
                    timeUnitName = "月";
                }
                ...
                sb.append("时间:"+list.get(i).get("Time") +timeUnitName+"<br>");
