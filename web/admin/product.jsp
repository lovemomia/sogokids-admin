            ...
            <div class="box span12">
                <div class="row-fluid sortable">
                    <div class="box-header well" data-original-title>
                        <h2><i class="icon-user"></i> 活动管理</h2>
                        <div class="box-icon">
                            <a href="${ctx}/product/oper.do?uid=${user.id}&id=0&mark=0&pageNo=${queryPage.pageNo}" class="btn btn-add btn-round"><i class=" icon-plus"></i></a>
                        </div>
                    </div>
                            ...
                            <table class="table table-striped table-bordered bootstrap-datatable datatable">
                                <c:forEach items="${queryPage.list}" var="entity">
                                    <tr>
                                        <td>
                                            ...
                                            图片编辑:
                                            <c:if test="${entity.status == 2}">
                                                <a class="btn btn-success" href="${ctx}/product/oper.do?uid=${user.id}&id=${entity.id}&mark=1&pageNo=${queryPage.pageNo}">
                                                    图片
                                                </a>
                                            </c:if><br>
                                            绑定库存单位状态:<c:choose>
                                                    <c:when test="${entity.skustatus == 0}">
                                                        未绑定sku
                                                    </c:when>
                                                    <c:otherwise>
                                                        已绑定sku
                                    ...
                                    <tr>
                                        <td class="center">
                                            ...
                                            <c:if test="${entity.status == 1}">
                                                <a class="btn btn-success" href="${ctx}/product/offline.do?uid=${user.id}&id=${entity.id}&pageNo=${queryPage.pageNo}">
                                                    下线
                                                </a>
                                            </c:if>
                                        </td>
                                ...
                                <tr>
                                    <td class="center">
                                        ...
                                        <a href="${ctx}/product/info.do?uid=${user.id}&pageNo=${queryPage.topPageNo}">
                                            首页
                                        </a>
                                        <a href="${ctx}/product/info.do?uid=${user.id}&pageNo=${queryPage.previousPageNo}">
                                            上一页
                                        </a>
                                        <a href="${ctx}/product/info.do?uid=${user.id}&pageNo=${queryPage.nextPageNo}">
                                            下一页
                                        </a>
                                        <a href="${ctx}/product/info.do?uid=${user.id}&pageNo=${queryPage.bottomPageNo}">
                                            尾页
                                        </a>
