            ...
            <div class="ibox-content">
                <form class="form-horizontal" id="create_form" action="" method="post">
                    <div class="form-group">
                        <div class="col-sm-4">
                            <input id="g_id" name="g_id" type="hidden" value="${model.id}">
                            <input id="mobiles" name="mobiles" type="text" class="form-control" placeholder="请输入手机号">
                        </div>
                        <div class="col-sm-1">
                            <button type="button" class="btn btn-warning" id="btn_create_save" name="btn_create_save">添加</button>
                        </div>
                    </div>
                </form>
                <form>
...
<script>
    $(document).ready(function () {
        $('#btn_create_save').click(function (){
            ...
            if(mobiles == null || mobiles == ""){
                ...
                $.post("/group/add_users.do", $("#create_form").serialize(),
                        function(data){
                            if(data.success == 0) {
                                $.layer({
                                    shade : ['',false],
                                    ...
                                    dialog : {msg:data.msg, btns : 1, type : 4, btn : ['确定'],
                                        yes : function(){
                                            window.location.href = "${ctx}/group/oper.do?uid=${user.id}&id="+gid+"&mark=2";
                                        }
                                    }
                                });
                            }else{
    ...
    function delUser(id){
        var gid = $('#g_id').val();
        layer.confirm('您确定要删除此成员吗？', function(index){
            window.location.href="${ctx}/group/del_users.do?uid=${user.id}&id="+id+"&gid="+gid;
            layer.close(index);
        });
    }
